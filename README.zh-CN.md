...
### 它是 install-ready 的，不只是 prompt
Clawin package 的设计目标是成为 OpenClaw 的可安装起点，而不只是一个角色想法或零散 prompt 片段。

### 它是为了“发现”而组织的
所有 agent 按行业、角色族、agent 身份组织，用户可以带着目的浏览，而不是盲猜从哪里开始。

### 它比从零搭建更快
Clawin 可以减少 blank-page friction。你不用手动设计每个 workspace 文件、package 结构和起始角色，可以先从一个结构化 package 开始，再按本地环境调整。
...
所以它不仅在“安装前”有价值，在“评估中”和“准备激活第一个 agent”时也有价值。
...
- **开发者和技术团队**：探索按角色组织的 agent 方案
- **运营、研究和业务团队**：按工作流寻找更结构化的 agent package
- **GitHub 资源收藏者**：希望收藏一个值得反复回看的 OpenClaw 生态入口

Clawin **不是**一个通用的零配置 AI App。它面向的是希望安装 OpenClaw agent package，并且希望更清晰地做选择的用户。
...
- 更快找到按角色细分的 agent
- 看到真实 package 结构，而不是零散 prompt 片段
- 当新工作流或新团队需求出现时，回来快速挑选起点
- 跟踪 Clawin 公共目录如何持续扩张

这个仓库的设计目标，就是让你在“还没安装前”也觉得它有收藏价值。
...
当前 full-catalog public release：
...
这个仓库是 Clawin 的 **public output catalog**。
它服务于这些目标：
- 浏览可安装的 OpenClaw agents
- 按角色和领域选择合适 package
- 把已发布 package 安装进自己的 OpenClaw 环境

## 这个仓库不是什么

这个仓库 **不包含** Clawin 的私有生产流水线。
它不包含：
- 原始角色源文档
- 规范化或生成脚本
- 内部 review 队列
