...
2. Open `config/openclaw.agent.content-platform-anti-scraping-and-bonus-abuse-strategist.entry.json`.
3. Append that JSON object to `agents.list` in the target machine's `~/.openclaw/openclaw.json`.
4. If you prefer a wrapper snippet, merge `config/openclaw.agent.content-platform-anti-scraping-and-bonus-abuse-strategist.snippet.json` into the target config instead.
