# Install Device and IP Risk Strategist
...
```text
~/.openclaw/agents/i5-device-ip-risk-strategist
```
...
## Steps
1. Copy this entire folder to `~/.openclaw/agents/i5-device-ip-risk-strategist`.
2. Open `config/openclaw.agent.i5-device-ip-risk-strategist.entry.json`.
3. Append that JSON object to `agents.list` in the target machine's `~/.openclaw/openclaw.json`.
4. If you prefer a wrapper snippet, merge `config/openclaw.agent.i5-device-ip-risk-strategist.snippet.json` into the target config instead.
...
```text
~/.openclaw/agents/i5-device-ip-risk-strategist/workspace/skills
```
...
- workspace: `~/.openclaw/agents/i5-device-ip-risk-strategist/workspace`
- agentDir: `~/.openclaw/agents/i5-device-ip-risk-strategist/agent`
