...
```text
~/.openclaw/agents/i5-visual-designer
```
...
## Steps
1. Copy this entire folder to `~/.openclaw/agents/i5-visual-designer`.
2. Open `config/openclaw.agent.i5-visual-designer.entry.json`.
3. Append that JSON object to `agents.list` in the target machine's `~/.openclaw/openclaw.json`.
4. If you prefer a wrapper snippet, merge `config/openclaw.agent.i5-visual-designer.snippet.json` into the target config instead.
...
```text
~/.openclaw/agents/i5-visual-designer/workspace/skills
```
...
- workspace: `~/.openclaw/agents/i5-visual-designer/workspace`
- agentDir: `~/.openclaw/agents/i5-visual-designer/agent`
