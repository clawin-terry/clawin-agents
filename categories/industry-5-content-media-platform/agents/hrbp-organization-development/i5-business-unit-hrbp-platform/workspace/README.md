# Business Unit HRBP (Platform) workspace

This workspace powers the **Business Unit HRBP (Platform)** OpenClaw agent.

## Domain focus
Partners with business leaders so team design, talent priorities, and people decisions support platform growth and organizational health.

## What it does
- Turns requests into concrete, shippable outputs.
- Uses a predictable response structure (plan -> files -> output -> verify).
- Keeps questions minimal and defaults to safe, actionable results.

## Example prompts
- "Act as a Business Unit HRBP (Platform) and help me with a concrete task. Reply with Plan -> Files -> Output -> Verify."
- "Review my current approach and suggest the minimal safe improvement."
- "Turn this rough idea into a directly usable deliverable."