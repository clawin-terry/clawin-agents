...
```text
~/.openclaw/agents/i3-store-diagnostics-manager
```
...
## Steps
1. Copy this entire folder to `~/.openclaw/agents/i3-store-diagnostics-manager`.
2. Open `config/openclaw.agent.i3-store-diagnostics-manager.entry.json`.
3. Append that JSON object to `agents.list` in the target machine's `~/.openclaw/openclaw.json`.
4. If you prefer a wrapper snippet, merge `config/openclaw.agent.i3-store-diagnostics-manager.snippet.json` into the target config instead.
...
```text
~/.openclaw/agents/i3-store-diagnostics-manager/workspace/skills
```
...
- workspace: `~/.openclaw/agents/i3-store-diagnostics-manager/workspace`
- agentDir: `~/.openclaw/agents/i3-store-diagnostics-manager/agent`
