...
```text
~/.openclaw/agents/i1-audit-analyst
```
...
## Steps
1. Copy this entire folder to `~/.openclaw/agents/i1-audit-analyst`.
2. Open `config/openclaw.agent.i1-audit-analyst.entry.json`.
3. Append that JSON object to `agents.list` in the target machine's `~/.openclaw/openclaw.json`.
4. If you prefer a wrapper snippet, merge `config/openclaw.agent.i1-audit-analyst.snippet.json` into the target config instead.
...
```text
~/.openclaw/agents/i1-audit-analyst/workspace/skills
```
...
- workspace: `~/.openclaw/agents/i1-audit-analyst/workspace`
- agentDir: `~/.openclaw/agents/i1-audit-analyst/agent`
