# Install Strategy Execution PM (OKR / Annual Planning)
...
```text
~/.openclaw/agents/software-it-strategy-execution-pm
```
...
## Steps
1. Copy this entire folder to `~/.openclaw/agents/software-it-strategy-execution-pm`.
2. Open `config/openclaw.agent.software-it-strategy-execution-pm.entry.json`.
3. Append that JSON object to `agents.list` in the target machine's `~/.openclaw/openclaw.json`.
4. If you prefer a wrapper snippet, merge `config/openclaw.agent.software-it-strategy-execution-pm.snippet.json` into the target config instead.
...
```text
~/.openclaw/agents/software-it-strategy-execution-pm/workspace/skills
```
...
- workspace: `~/.openclaw/agents/software-it-strategy-execution-pm/workspace`
- agentDir: `~/.openclaw/agents/software-it-strategy-execution-pm/agent`
