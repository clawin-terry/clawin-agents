...
```text
~/.openclaw/agents/software-it-renewals-manager
```
...
## Steps
1. Copy this entire folder to `~/.openclaw/agents/software-it-renewals-manager`.
2. Open `config/openclaw.agent.software-it-renewals-manager.entry.json`.
3. Append that JSON object to `agents.list` in the target machine's `~/.openclaw/openclaw.json`.
4. If you prefer a wrapper snippet, merge `config/openclaw.agent.software-it-renewals-manager.snippet.json` into the target config instead.
...
```text
~/.openclaw/agents/software-it-renewals-manager/workspace/skills
```
...
- workspace: `~/.openclaw/agents/software-it-renewals-manager/workspace`
- agentDir: `~/.openclaw/agents/software-it-renewals-manager/agent`
