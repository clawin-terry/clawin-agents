...
2. Open `config/openclaw.agent.software-it-partner-recruitment-manager.entry.json`.
3. Append that JSON object to `agents.list` in the target machine's `~/.openclaw/openclaw.json`.
4. If you prefer a wrapper snippet, merge `config/openclaw.agent.software-it-partner-recruitment-manager.snippet.json` into the target config instead.
