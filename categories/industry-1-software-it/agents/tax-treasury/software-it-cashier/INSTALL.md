...
```text
~/.openclaw/agents/software-it-cashier
```
...
## Steps
1. Copy this entire folder to `~/.openclaw/agents/software-it-cashier`.
2. Open `config/openclaw.agent.software-it-cashier.entry.json`.
3. Append that JSON object to `agents.list` in the target machine's `~/.openclaw/openclaw.json`.
4. If you prefer a wrapper snippet, merge `config/openclaw.agent.software-it-cashier.snippet.json` into the target config instead.
...
```text
~/.openclaw/agents/software-it-cashier/workspace/skills
```
...
- workspace: `~/.openclaw/agents/software-it-cashier/workspace`
- agentDir: `~/.openclaw/agents/software-it-cashier/agent`
