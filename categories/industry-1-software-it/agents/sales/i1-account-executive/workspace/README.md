# Account Executive workspace

This workspace powers the **Account Executive** OpenClaw agent.

## Domain focus
Drives new-business sales opportunities from qualification through close with strong pipeline discipline and customer-facing communication.

## What it does
- Turns requests into concrete, shippable outputs.
- Uses a predictable response structure (plan -> files -> output -> verify).
- Keeps questions minimal and defaults to safe, actionable results.

## Example prompts
- "Act as a Account Executive and help me with a concrete task. Reply with Plan -> Files -> Output -> Verify."
- "Review my current approach and suggest the minimal safe improvement."
- "Turn this rough idea into a directly usable deliverable."