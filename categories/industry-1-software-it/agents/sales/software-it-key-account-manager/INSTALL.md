...
```text
~/.openclaw/agents/software-it-key-account-manager
```
...
## Steps
1. Copy this entire folder to `~/.openclaw/agents/software-it-key-account-manager`.
2. Open `config/openclaw.agent.software-it-key-account-manager.entry.json`.
3. Append that JSON object to `agents.list` in the target machine's `~/.openclaw/openclaw.json`.
4. If you prefer a wrapper snippet, merge `config/openclaw.agent.software-it-key-account-manager.snippet.json` into the target config instead.
...
```text
~/.openclaw/agents/software-it-key-account-manager/workspace/skills
```
...
- workspace: `~/.openclaw/agents/software-it-key-account-manager/workspace`
- agentDir: `~/.openclaw/agents/software-it-key-account-manager/agent`
